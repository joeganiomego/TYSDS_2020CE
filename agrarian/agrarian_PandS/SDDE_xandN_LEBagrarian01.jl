...
Freducr : 0.9 ; fertility reduction .  0.9 instead 1.0 because with advanced agrarian society some artificial fertility reduction can be expected.
...
KtoX : 2.47; constant linking E and P numbers to the x output.
keju : 0.1 ;   keju*N = SN , people that works for the state.
fleecing : 0.00001 ; constant to account for a continous loss of bioenergy.
xx0 : 0.20 ; minimal value of bioenergy at which bioenergy output goes to 0.
...
# isoclines color ###############
plt1 = plot(layout=(1,1),show = true,title="keju_xxn_NNo_LEBagrarian, SN=0.1",size=(600,400))
...
#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_1")

plot!(plt1[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_1big")
...
#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_bw_1")

plot!(plt1[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_bw_1big")
...
u0 = [1.1, 0.9]; #Initial populations
tspan = (0.0,dur);
...
CSV.write("xandN_agrarian01_ODE.csv",sol)
CSV.write("xandN_agrarian01_SDE.csv",SDEsol)
CSV.write("xandN_agrarian01_DDE.csv",DDEsol)
CSV.write("xandN_agrarian01_SDDE.csv",SDDEsol)
...
#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_2")

plot!(plt1[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_2big")
...
#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_bw_2")

plot!(plt1[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_bw_2big")
...
plt2 = plot(layout=(1,1),title="keju_xxn_NNo_LEBagrarian, SN=0.1",size=(600,400))
...
#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_3")

plot!(plt2[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_3big")
...
#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_bw_3")

plot!(plt2[1],size=(1000,600),show = true)

#read(stdin, Char)
savefig("SDDE_xandN_LEBagrarian01_bw_3big")
